...
export function AppSidebar() {
  ...
  const getNavClasses = (path: string) => {
    ...
    return active
      ? "bg-dashboard-primary text-primary-foreground font-medium hover:bg-dashboard-primary hover:!text-primary-foreground"
      ...
      <SidebarContent className="border-r border-dashboard-primary/10">
        <div className="p-4 border-b border-dashboard-primary/10">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-dashboard-primary to-dashboard-muted flex items-center justify-center">
              <BarChart3 className="w-4 h-4 text-primary-foreground" />
            </div>
            {!collapsed && (
              <div>
                <h1 className="font-semibold text-foreground">News Analytics</h1>
              ...
              {allItems.map((item) => (
                <div key={item.title}>
                  {item.submenu ? (
                    <>
                      <SidebarMenuItem>
                        <SidebarMenuButton className="h-10" onClick={() => toggleMenu(item.title)}>
                          ...
                          {item.submenu.map((subItem) => (
                            <SidebarMenuItem key={subItem.title}>
                              <SidebarMenuButton asChild className="h-8">
                                <NavLink to={subItem.url} className={getNavClasses(subItem.url)}>
