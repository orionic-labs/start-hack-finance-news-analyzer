    ...
    if (total > 100) {
      ...
      if (otherTotal > 0) {
        otherKeys.forEach((otherKey) => {
          const currentVal =
            typeof newPortfolio[otherKey] === "number"
              ? newPortfolio[otherKey]
              : parseFloat(String(newPortfolio[otherKey])) || 0;
          const proportion = currentVal / otherTotal;
          newPortfolio[otherKey] = Math.max(
            0,
            currentVal - excess * proportion
          );
        });
      }
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-lg">
            Manage client portfolios and asset allocations
          </p>
        </div>
                ...
                <div className="flex-1 space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Button
                      ...
                      size="lg"
                      onClick={() =>
                        handleEditPortfolio(client.id, client.portfolio)
                      }
                      className="border-primary/20 hover:border-primary/40 hover:bg-primary/5 hover:text-card-foreground px-6 py-3 transition-all duration-300 hover:shadow-md hover:scale-105"
                    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-muted/5 rounded-xl p-6 border border-primary/10">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {Object.entries(client.portfolio).map(
                          ([key, value]) => (
                            <div
                              key={key}
                            ...
                            >
                              <span className="text-sm text-muted-foreground font-medium">
                                {
                                  portfolioLabels[
                                    key as keyof typeof portfolioLabels
                                  ]
                                }
                              </span>
                              <div className="flex items-center gap-2">
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-hidden bg-gradient-to-br from-background to-muted/20">
          <DialogHeader className="pb-4 border-b border-primary/10">
            ...
            <p className="text-muted-foreground">
              Adjust the portfolio allocation percentages for this client
            </p>
          </DialogHeader>
          ...
          <div className="space-y-6 max-h-96 overflow-y-auto p-1">
            <div className="grid gap-4">
              {Object.entries(portfolioLabels).map(([key, label]) => (
                ...
                >
                  <Label className="text-sm font-medium text-foreground flex-1 pr-4">
                    {label}
                  </Label>
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <Input
                        ...
                        onChange={(e) =>
                          handlePortfolioChange(key, e.target.value)
                        }
                    ...
                    <div className="w-16 h-2 bg-muted/50 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(editPortfolio[key] || 0, 100)}%`,
                        }}
                      />
                    </div>
                  </div>
